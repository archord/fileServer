 ...
 */
@Repository
public class TemplateCatalogDaoImpl extends BaseHibernateDaoImpl<TemplateCatalog> implements TemplateCatalogDao {
  
  @Override
  public Boolean exist(TemplateCatalog obj) {
    Boolean flag = false;
    Session session = getCurrentSession();
    String sql = "select tc_id from template_catalog where name='" + obj.getName() + "' ";
    Query q = session.createSQLQuery(sql);
    if (!q.list().isEmpty()) {
      BigInteger cfId = (BigInteger) q.list().get(0);
      obj.setTcId(cfId.longValue());
      flag = true;
    }
    return flag;
  }
